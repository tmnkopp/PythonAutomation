...
class picklist_recommender():
    def __init__(self, ctx, use_cache=True, picklist_where='1=1'):  
        self.ctx=ctx
    ...
    def to_cache(self, fname='pl.json'):
        with open( self.ctx.get_cache_dir()+'\\'+fname, 'w') as f:
            f.write(json.dumps(self.cache, cls=NpEncoder, indent=2))
            #json.dump(self.cache, f, cls=NpEncoder, indent=2)

    def load_cache(self, fname='pl.json'):
        dir=self.ctx.get_cache_dir()+'\\'+fname
        if not exists(dir):
            return {}
    ...
    def normalize(self,s,length=15 ):   
        s=s.upper().strip() 
        s=re.sub('[^A-Z0-9]','',s)
        if len(s) > length:
            s=re.sub('[AEI]','',s)
